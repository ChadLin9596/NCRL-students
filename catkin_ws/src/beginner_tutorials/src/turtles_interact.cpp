	...
	void follow(turtles& turtles1,turtlesim::Pose& host_mocap,geometry_msgs::Twist*t,displacement &dis_host,turtlesim::Pose& nbr_mocap,displacement &dis_nbr,turtlesim::Pose&nbr_mocap2,displacement&dis_nbr2,turtlesim::Pose& nbr_mocap3,displacement &dis_nbr3)
{
		...
		errx = errx1;  
		//errx=0.1*errx1+0.3*errx2+0.3*errx3+0.3*errx4;
		...
		erry=erry1;  
		//erry=0.1*erry1+0.3*erry2+0.3*erry3+0.3*erry4;
		...
		err_theta=atan2 (erry1,errx1); //-pi~pi
		err_theta2=atan2(erry2,errx2);
		err_theta3=atan2(erry3,errx3);
		err_theta4=atan2(erry4,errx4);
		...
		ROS_INFO("long %f,theta %f,h.theta %f ",err_long,err_theta/pi*180, host_mocap.theta/pi*180);
if(err_long<0.1) //&& err_theta<0.01)
		{
			err_theta=0;
			err_long=0;
		}
	   ...
	   /* ROS_INFO("err_theta1 %f err_theta2 %ferr_theta3 %f err_theta4 %f",err_theta/pi*180,err_theta2/pi*180,err_theta3/pi*180,err_theta4/pi*180);
	...
	void follow(turtles& turtles1,turtlesim::Pose& host_mocap,geometry_msgs::Twist*t,displacement &dis_host,turtlesim::Pose& nbr_mocap,displacement &dis_nbr,turtlesim::Pose&nbr_mocap2,displacement&dis_nbr2,turtlesim::Pose& nbr_mocap3,displacement &dis_nbr3)
{
		...
		//errx = errx1;  
		errx=0.7*errx1+0.1*errx2+0.1*errx3+0.1*errx4;
		...
		erry4 = nbr_mocap3.y-host_mocap.y+local_y3;
		//erry=erry1;  
		erry=0.7*erry1+0.1*erry2+0.1*erry3+0.1*erry4;
	    ...
	    //err_theta= 0.25*err_theta1- host_mocap.theta+0.25*err_theta2+0.25*err_theta3+0.25*err_theta4;
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "turtles_interact");
