	...
	void follow(turtles& turtles1,turtlesim::Pose& host_mocap,geometry_msgs::Twist*t,displacement &dis_host,turtlesim::Pose& nbr_mocap,displacement &dis_nbr,turtlesim::Pose&nbr_mocap2,displacement&dis_nbr2,turtlesim::Pose& nbr_mocap3,displacement &dis_nbr3)
{
		...
		host_mocap.theta = host_mocap.theta - 2*pi;	
	   	...
	   	err_theta= 0.1*err_theta1- host_mocap.theta+0.3*err_theta2+0.3*err_theta3+0.3*err_theta4;
	   	//err_theta= (err_theta1+err_theta2+err_theta3+err_theta4-host_mocap.theta)/3;

		if(err_theta>pi)
		err_theta=err_theta-2*pi;
		else if(err_theta1<-pi)
		err_theta=err_theta+2*pi; //turn to -pi~pi*/
		

		//err_theta=-pi; //return when bump the wall
	...
	//	if((0.5<turtles1.x<11.5)||(0.5<turtles1.y<11.5))
		ux = KPx*err_long;
		/*else
		ux=0;
		...
		//ROS_INFO("%f,%f,%f",err_theta1/pi*180,err_theta1/pi*180,host_mocap.theta/pi*180); 	
